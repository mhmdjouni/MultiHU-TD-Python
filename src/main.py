...
from src.ao_admm import AOADMMASC, AOADMMASCNaive, CPDAOADMM
...
def main_aoadmm():
    ...
    rec_error = la.norm(factors_orig[1] - ao_admm.factors[1]) / la.norm(factors_orig[1])
    ...
    rec_error = la.norm(factors_orig[2] - ao_admm.factors[2]) / la.norm(factors_orig[2])
    print(f"Reconstruction error: {rec_error}")
    ...
    rec_error = la.norm(factors_orig[0] - ao_admm.factors[0]) / la.norm(factors_orig[0])
    print(f"Reconstruction error: {rec_error}")
