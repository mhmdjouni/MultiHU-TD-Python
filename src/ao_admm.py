...
class AbstractCPDAOADMM(ABC):
    ...
    @abstractmethod
    def __postprocessing_normalize(self):
        ...

    @abstractmethod
    def __postprocessing_sort_columns(self):
        ...
...
class AOADMMASC(AbstractCPDAOADMM):
    ...
    def run(self, bsum: float = 0):
        # Initialize
        self.factors, tensor_unfoldings, dual_vars = self.__initialize()
        ...
        for itr in range(self.n_iters):
            ...
            tensor_unfoldings = self.__asc_update(delta=self.tensor_mean,)

        # Restore the tensor and factor matrices to their original states
        self.__restore()

        # Normalize the factors into the diagonal entries
        self.__postprocessing_normalize()

        # Sort the factors by the diagonal entries in descending order
        self.__postprocessing_sort_columns()


@dataclass
class AOADMMASCNaive(AbstractCPDAOADMM):
    """
    Solve CPD using AO (mainly through ADMM)
    Future development may include:
     - Loris-Verhoeven solver for the factor matrices
